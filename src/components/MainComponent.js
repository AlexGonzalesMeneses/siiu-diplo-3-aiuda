...
import NewContact from './newContactComponent';
...
export function Main() {
  ...
  const ItemWithId = (props) => {
    ...
    return (
      <ItemDetail
        item={items.filter((item) => item.id === parseInt(itemId, 10))[0]}
  ...
  return (
    <div>
      ...
      <Routes>
        ...
        />
        <Route exact path='catalog' element={<Catalog items={items} />}></Route>
        <Route path='catalog/:itemId' element={<ItemWithId />} />
        <Route exact path='/contactus' element={<NewContact />} />
        <Route
          exact
          path='/aboutus'
          element={<About employees={employees} />}
        />
        <Route
          path='*'
          element={
